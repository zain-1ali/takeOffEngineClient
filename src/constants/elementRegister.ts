 ...
 *
 * Keep in sync with backend/src/services/reports/elementRegister.ts
 */
...
export type ElementRegisterEntry = {
  ...
  /** Primary billed unit */
  primaryUnit: string
  /** Secondary quantities the engine / BOQ also produces */
  secondaryQuantities: string[]
  /** Measurement rule / basis of measure */
  measurementRule: string
  /** Default material or specification family */
  defaultMaterial: string
  takeoffMethod: TakeoffMethod
  /** NRM2 work-section style reference (indicative) */
  nrm2Ref: string
  /**
   * Lower rank owns shared volume where two elements intersect.
   * Finishes / MEP sit high (lose) so structure keeps concrete volume.
   */
  overlapRank: number
  implemented: boolean
...
/**
 * 24 element codes: M1 01–12 (incl. 02a), M2 13–19, M3 20–23.
 */
...
/** Element that owns shared volume (lower overlapRank wins). */
